...
JNIEXPORT jboolean JNICALL Java_com_android_canlibrary_interfaces_CanInterface_openCan2Interface(JNIEnv *env, jobject obj, jstring deviceName) {
    const char *cDeviceName = (*env)->GetStringUTFChars(env, deviceName, NULL);
...
JNIEXPORT jboolean JNICALL Java_com_android_canlibrary_interfaces_CanInterface_sendCan2Message(JNIEnv *env, jobject obj, jint canId, jbyteArray data) {
    // 获取数据数组长度
    jsize dataLength = (*env)->GetArrayLength(env, data);
...
JNIEXPORT jbyteArray JNICALL Java_com_android_canlibrary_interfaces_CanInterface_receiveCan2Message(JNIEnv *env, jobject obj) {
    // 创建一个字节数组来存储接收到的数据
    jbyteArray receivedData = (*env)->NewByteArray(env, 16);
...
JNIEXPORT void JNICALL Java_com_android_canlibrary_interfaces_CanInterface_closeCan2Interface(JNIEnv *env, jobject obj) {
    // 关闭CAN设备文件
    if (canFd2 >= 0) {
        close(canFd2);
        canFd2 = -1;
